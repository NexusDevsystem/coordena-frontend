...
function solicitarPermissaoNotificacao() {
  if (!("Notification" in window))
    return console.warn("Sem API de Notificações.");
  ...
  if (Notification.permission !== "denied") {
    Notification.requestPermission().then(
      (p) => (notificacoesAtivas = p === "granted")
    );
  }
}
...
const Api = (() => {
  ...
  function authHeaders(isJson = false) {
    // token de usuário comum (Auth) OU token de admin salvo no painel
    const userToken =
  ...
  return {
    ...
    deleteEvent: (id) =>
      fetchJson(`${BASE}/${id}`, { method: "DELETE", headers: authHeaders() }),
  };
})();
...
const CalendarModule = (() => {
  ...
  async function reloadEvents() {
    try {
      ...
      calendar.getEvents().forEach((fc) => {
        if (fc.rendering !== "background") fc.remove();
      });
      events = approved;
      approved.forEach((ev) => {
        calendar.addEvent({
...
const FormModule = (() => {
  ...
  function open(id = null, evData = null) {
    ...
    selectors.fields.materia.innerHTML =
    ...
    if (evData) {
      ...
      if (evData.responsible) {
        ...
        f.resp.setAttribute("readonly", "readonly");
      }
  ...
  function init() {
    ...
    selectors.fields.start.addEventListener("change", () => {
      ...
      const d = new Date();
      d.setHours(hh, mm + 50);
      selectors.fields.end.value = `${String(d.getHours()).padStart(
        2,
        "0"
...
async function buildOccupancyTable(filterDate) {
  ...
  const slotStart = toDate(Y, M, D, "08:00"),
    slotEnd = toDate(Y, M, D, "22:00");
  const timeRanges = [];
  let cur = new Date(slotStart);
  while (cur < slotEnd) {
    const next = new Date(cur);
    next.setMinutes(cur.getMinutes() + 50);
  ...
  thead.innerHTML = `<tr><th class="px-2 py-1 border ${textClass}">Sala / Horário</th>${timeRanges
    .map(
      (r) => `<th class="px-2 py-1 border ${textClass} text-center">${r}</th>`
    )
    .join("")}</tr>`;
  ...
  labs.forEach((lab) => {
    ...
    timeRanges.forEach((range) => {
      ...
      if (hasReservation) {
        ...
        const c = turnoColors[fixed.turno] || "rgba(107,114,128,0.5)";
...
async function refreshEvents() {
  try {
    ...
    CalendarModule.getEvents()
      .slice()
      .forEach((e) => CalendarModule.remove(e._id));
    updated.forEach((e) => CalendarModule.add(e));
  } catch (e) {
    console.error("Buscar eventos:", e);
  }
}
...
async function initOccupancyUpdates() {
  try {
    ...
    console.error("fixedSchedules:", e);
  }
...
onReady(async () => {
  ...
  if ("Notification" in window && Notification.permission === "granted") {
    ...
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = '<i class="fas fa-bell-slash"></i> Notificações Ativadas';
    }
  }
  ...
  try {
    ...
    console.warn("Falha buscar reservas, iniciando vazio", e);
  }
  ...
  if (!dateInput) console.error("Elemento #occupancy-table não encontrado!");
    ...
    if (document.getElementById("calendar")) {
      CalendarModule.init(
        ...
        (info) => {
          const ev = CalendarModule.getEvents().find(
            (e) => e._id === info.event.id
          );
...
async function carregarHistoricoUsuarios() {
  ...
  if (!container)
    return console.error("#lista-historico-usuarios não encontrado");
  container.innerHTML =
    '<p class="text-center py-4">Carregando histórico...</p>';
  try {
    ...
    const termoBusca =
      document
        .getElementById("busca-historico-usuarios")
        ?.value.trim()
        .toLowerCase() || "";
    const ordenacao =
      document.getElementById("ordenacao-historico-usuarios")?.value ||
      "updatedAt";
    ...
    filtrados.forEach((u) => {
      html += `<div class="col-12 col-md-6 col-lg-4"><div class="card shadow-sm h-100"><div class="card-body">
        ...
        <p class="card-text mb-1"><small>Função: <strong>${
          u.role
        }</strong></small></p>
        ...
        <p class="card-text text-muted small">Data de cadastro: ${new Date(
          u.createdAt
        ).toLocaleDateString("pt-BR")}<br>Última atualização: ${new Date(
        u.updatedAt
...
document
  ...
  ?.addEventListener("shown.bs.tab", carregarHistoricoUsuarios);
document
  .getElementById("busca-historico-usuarios")
  ?.addEventListener("input", carregarHistoricoUsuarios);
document
  .getElementById("ordenacao-historico-usuarios")
  ?.addEventListener("change", carregarHistoricoUsuarios);
...
(function () {
  ...
  async function carregarUsuariosPendentes() {
    try {
      ...
      if (ultimoCountUsuarios === null && dados.length > 0) {
        ...
        ultimoCountUsuarios !== null &&
        dados.length > ultimoCountUsuarios
      ) {
      ...
      console.error("carregarUsuariosPendentes:", e);
    }
  }
  ...
  function renderizarUsuariosPendentes() {
    ...
    if (!container) return;
    const busca =
      document.getElementById("busca-usuarios")?.value.trim().toLowerCase() ||
      "";
    const ordenacao =
      document.getElementById("ordenacao-usuarios")?.value || "createdAt";

    let filtrados = usuariosPendentes.filter((u) =>
      ((u.name || "") + " " + (u.email || "")).toLowerCase().includes(busca)
    );
    filtrados.sort((a, b) =>
      ordenacao === "createdAt"
    ...
    exibidos.forEach((u) => {
      ...
      html += `<div class="col-md-6 col-lg-4"><div class="card card-coordena shadow-sm"><div class="card-body">
        <div class="d-flex justify-content-between align-items-start mb-2">
          <div><h5 class="card-title mb-1">${
            u.name
          }</h5><h6 class="card-subtitle mb-1">${u.email}</h6></div>
          <span class="badge bg-warning text-dark rounded-pill">Pendente</span>
        </div>
        <p class="mb-1"><i class="fas fa-user-tag me-1"></i><strong>Tipo:</strong> ${
          u.role
        }</p>
        <p class="mb-3"><i class="fas fa-calendar-alt me-1"></i><strong>Criado em:</strong> ${new Date(
          u.createdAt
    ...
    if (totalPag > 1) {
      html += `<nav aria-label="Paginação de Usuários" class="mt-4"><ul class="pagination justify-content-center">
        <li class="page-item ${
          paginaAtualUsuarios === 1 ? "disabled" : ""
        }"><a class="page-link" href="#" onclick="mudarPaginaUsuarios(${
        paginaAtualUsuarios - 1
      })">&laquo;</a></li>`;
      for (let p = 1; p <= totalPag; p++)
        html += `<li class="page-item ${
          paginaAtualUsuarios === p ? "active" : ""
        }"><a class="page-link" href="#" onclick="mudarPaginaUsuarios(${p})">${p}</a></li>`;
      html += `<li class="page-item ${
        paginaAtualUsuarios === totalPag ? "disabled" : ""
      }"><a class="page-link" href="#" onclick="mudarPaginaUsuarios(${
        paginaAtualUsuarios + 1
      })">&raquo;</a></li></ul></nav>`;
    }
  ...
  function mudarPaginaUsuarios(p) {
    ...
    renderizarUsuariosPendentes();
    document
      .getElementById("lista-pendentes-usuarios")
  ...
  async function aprovarUsuario(id) {
    ...
    const ok = await adminFetch(`${BASE_API}/api/admin/approve-user/${id}`, {
      method: "PATCH",
    });
  ...
  async function rejeitarUsuario(id) {
    ...
    const ok = await adminFetch(`${BASE_API}/api/admin/reject-user/${id}`, {
      method: "PATCH",
    });
    if (ok !== null) carregarUsuariosPendentes();
  }
  ...
  async function carregarReservasPendentes() {
    try {
      const dados = await adminFetch(
        `${BASE_API}/api/admin/pending-reservations`
      );
      if (!dados) return;
      const podeNotificar =
        typeof enviarNotificacao === "function" &&
      ...
      if (ultimoCountReservas === null && dados.length > 0) {
        ...
        ultimoCountReservas !== null &&
        dados.length > ultimoCountReservas
      ) {
      ...
      console.error("carregarReservasPendentes:", e);
    }
  }
  ...
  function renderizarReservasPendentes() {
    ...
    if (!container) return;
    const busca =
      document.getElementById("busca-reservas")?.value.trim().toLowerCase() ||
      "";
    const filtroData =
      document.getElementById("filtro-data-reservas")?.value || "";
    const ordenacao =
      document.getElementById("ordenacao-reservas")?.value || "date";
    ...
    exibidos.forEach((r) => {
      ...
      html += `<div class="col-md-6 col-lg-4"><div class="card card-coordena shadow-sm"><div class="card-body">
        <div class="d-flex justify-content-between align-items-start mb-2">
          <div><h5 class="card-title mb-1">${r.resource}${
        r.sala ? " – " + r.sala : ""
      }</h5><h6 class="card-subtitle mb-1">${new Date(
        r.date
      ).toLocaleDateString("pt-BR")}</h6></div>
          <span class="badge bg-warning text-dark rounded-pill">Pendente</span>
        </div>
        <p class="mb-1"><i class="fas fa-clock me-1"></i><strong>Horário:</strong> ${
          r.start
        } – ${r.end}</p>
        <p class="mb-1"><i class="fas fa-user me-1"></i><strong>Requisitante:</strong> ${
          r.responsible
        }</p>
        <p class="mb-1"><i class="fas fa-building me-1"></i><strong>Depto.:</strong> ${
          r.department
        }</p>
        <p class="mb-1"><i class="fas fa-info-circle me-1"></i><strong>Tipo:</strong> ${
          r.type
        }</p>
    ...
    if (totalPag > 1) {
      ...
      <li class="page-item ${
        paginaAtualReservas === 1 ? "disabled" : ""
      }"><a class="page-link" href="#" onclick="mudarPaginaReservas(${
        paginaAtualReservas - 1
      })">&laquo;</a></li>`;
      for (let p = 1; p <= totalPag; p++)
        html += `<li class="page-item ${
          paginaAtualReservas === p ? "active" : ""
        }"><a class="page-link" href="#" onclick="mudarPaginaReservas(${p})">${p}</a></li>`;
      html += `<li class="page-item ${
        paginaAtualReservas === totalPag ? "disabled" : ""
      }"><a class="page-link" href="#" onclick="mudarPaginaReservas(${
        paginaAtualReservas + 1
      })">&raquo;</a></li></ul></nav>`;
    }
  ...
  function mudarPaginaReservas(p) {
    ...
    renderizarReservasPendentes();
    document
      .getElementById("lista-pendentes-reservas")
      ?.scrollIntoView({ behavior: "smooth" });
  }
  ...
  async function aprovarReserva(id) {
    ...
    const ok = await adminFetch(
      `${BASE_API}/api/admin/approve-reservation/${id}`,
      { method: "PATCH" }
    );
    if (ok !== null) {
  ...
  async function rejeitarReserva(id) {
    ...
    const ok = await adminFetch(
      `${BASE_API}/api/admin/reject-reservation/${id}`,
      { method: "DELETE" }
    );
  ...
  async function concluirReservation(id) {
    try {
      await adminFetch(`${BASE_API}/api/reservations/${id}`, {
        method: "PUT",
        body: JSON.stringify({ status: "concluido" }),
      });
    } catch (e) {
      console.error(`concluir ${id}:`, e);
    }
  }

  async function carregarReservasAtivas() {
    try {
      const todas = await adminFetch(
        `${BASE_API}/api/reservations?status=approved`,
        { method: "GET" }
      );
      if (!todas) return;
      ...
      const termo =
        document.getElementById("busca-ativas")?.value.trim().toLowerCase() ||
        "";
      const filtroData =
        document.getElementById("filtro-data-ativas")?.value || "";
      const filtradas = todas
        .filter((r) => {
          const fim = new Date(`${r.date}T${r.end}:00`);
          if (agora > fim) return false;
          if (filtroData && r.date !== filtroData) return false;
          const lab = (r.sala || r.resource || "").toLowerCase(),
            resp = (r.responsible || "").toLowerCase();
          if (termo && !lab.includes(termo) && !resp.includes(termo))
            return false;
          return true;
        })
        .sort(
          (a, b) =>
            new Date(`${a.date}T${a.start}:00`) -
            new Date(`${b.date}T${b.start}:00`)
        );

      renderizarReservasAtivas(filtradas);
    } catch (e) {
      console.error("Reservas Ativas:", e);
    }
  }
  ...
  function renderizarReservasAtivas(reservas) {
    ...
    reservas.forEach((r) => {
      ...
      body.innerHTML = `
        ...
        <p class="card-text text-muted small">${inicio.toLocaleDateString(
          "pt-BR"
        )} &nbsp;|&nbsp; ${r.start} – ${r.end}</p>
        <div class="progress mt-3" style="height:8px;"><div class="progress-bar bg-success" role="progressbar" style="width:${perc}%;" aria-valuenow="${perc.toFixed(
        2
      )}" aria-valuemin="0" aria-valuemax="100"></div></div>
        <p class="text-end text-sm mt-1"><small>${perc.toFixed(
          0
        )}%</small></p>`;
  ...
  document
    .getElementById("busca-ativas")
    ?.addEventListener("input", carregarReservasAtivas);
  document
    .getElementById("filtro-data-ativas")
    ?.addEventListener("change", carregarReservasAtivas);
  ...
  document
    .getElementById("ordenacao-usuarios")
  ...
  document
    .getElementById("ordenacao-reservas")
    ?.addEventListener("change", () => {
      paginaAtualReservas = 1;
      renderizarReservasPendentes();
    });
  document
    .getElementById("filtro-data-reservas")
    ?.addEventListener("change", () => {
      paginaAtualReservas = 1;
      renderizarReservasPendentes();
    });
...
// ──────────────────────────────────────────────────
{
  ...
  btn2?.addEventListener("click", () => {
    ...
    localStorage.setItem(
      "theme",
      root2.classList.contains("dark") ? "dark" : "light"
    );
  });
}
...
onReady(() => {
  ...
  document.getElementById("password")?.addEventListener("input", () => {
    ...
    if (pwd.length >= 8)
      cMin?.classList.replace("text-red-500", "text-green-500");
    else cMin?.classList.replace("text-green-500", "text-red-500");
    if (/[a-z]/.test(pwd) && /[A-Z]/.test(pwd))
      cCase?.classList.replace("text-red-500", "text-green-500");
    else cCase?.classList.replace("text-green-500", "text-red-500");
    if (/\d/.test(pwd) || /[!@#$%^&*(),.?":{}|<>]/.test(pwd))
      cNumSym?.classList.replace("text-red-500", "text-green-500");
    ...
    else cNoMail?.classList.replace("text-red-500", "text-green-500");
    if (commonPasswords.includes(pwd.toLowerCase()))
      cCommon?.classList.replace("text-green-500", "text-red-500");
    else cCommon?.classList.replace("text-red-500", "text-green-500");
  });
});
