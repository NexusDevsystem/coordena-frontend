...
function enviarNotificacao(titulo, texto) {
  if (notificacoesAtivas && Notification.permission === 'granted') {
    new Notification(titulo, { body: texto, icon: '/assets/img/logo-notification.png' });
  }
}
...
const ThemeToggle = (() => {
  ...
  function applyTheme(mode) { root.classList.toggle('dark', mode === 'dark'); localStorage.setItem(themeKey, mode); }
  function init() {
...
// --------------------------------------------------
// API (reservas e horários fixos) - token de usuário comum
// --------------------------------------------------
const Api = (() => {
...
const CalendarModule = (() => {
  ...
  async function loadFixedSchedules() {
    ...
    } catch (e) { console.error('Horários fixos:', e); }
  }
  ...
  async function reloadEvents() {
    ...
    } catch (e) { console.error('Recarregar eventos:', e); }
  }
  ...
  function init(rawEvents, onDateClick, onEventClick) {
    ...
    calendar = new FullCalendar.Calendar(el, {
      ...
      allDaySlot: false,
      selectable: true,
      selectAllow: sel => !calendar.getEvents().some(ev => ev.rendering === 'background' && ev.start < sel.end && ev.end > sel.start)
    });
  ...
  return {
    init,
    add(ev) { events.push(ev); calendar.addEvent({ id: ev._id, title: `${ev.title} (${ev.time})`, start: `${ev.date}T${ev.start}`, end: `${ev.date}T${ev.end}` }); },
    update(id, ev) {
      const i = events.findIndex(x => x._id === id); if (i !== -1) events[i] = ev;
      const fc = calendar.getEventById(id);
      if (fc) { fc.setProp('title', `${ev.title} (${ev.time})`); fc.setStart(`${ev.date}T${ev.start}`); fc.setEnd(`${ev.date}T${ev.end}`); }
    },
    remove(id) { events = events.filter(x => x._id !== id); calendar.getEventById(id)?.remove(); },
...
const FormModule = (() => {
  ...
  function cacheSelectors() {
    ...
    selectors.fields = {
      data: $id('data'), start: $id('start'), end: $id('end'),
      recurso: $id('recurso'),
  ...
  function open(id = null, evData = null) {
    ...
    if (evData) {
      ...
      f.data.value = evData.date; f.start.value = evData.start; f.end.value = evData.end;
      f.recurso.value = evData.resource; f.recurso.dispatchEvent(new Event('change'));
      f.sala.value = evData.sala || ''; f.type.value = evData.type;
      if (evData.responsible) { f.resp.value = evData.responsible; f.resp.setAttribute('readonly', 'readonly'); }
      f.dept.value = evData.department; f.status.value = evData.status; f.desc.value = evData.description;
      if (evData.materia) { f.materia.innerHTML = `<option value="${evData.materia}">${evData.materia}</option>`; f.materia.disabled = false; }
    }
  ...
  async function handleSubmit(e) {
    ...
    const f = selectors.fields;
    const payload = {
      date: f.data.value, start: f.start.value, end: f.end.value,
      resource: f.recurso.value,
      sala: f.salaContainer.classList.contains('hidden') ? '' : f.sala.value,
      type: f.type.value, responsible: f.resp.value, department: f.dept.value,
      materia: f.materia.value, status: 'pending', description: f.desc.value,
    ...
    try {
      ...
      close();
    } catch (err) { alert('Erro ao criar/atualizar reserva: ' + err.message); }
  }
  ...
  function init() {
    ...
    const salaOpts = { 'Laboratório': ['Lab B401','Lab B402','Lab B403','Lab B404','Lab B405','Lab B406','Lab Imaginologia'] };
    ...
    const courseMap = {
      'Engenharia de Computação': [
        'ARA0003 - PRINCÍPIOS DE GESTÃO','ARA0017 - INTRODUCAO A PROGRAMAÇÃO DE COMPUTADORES','ARA0039 - ARQUITETURA DE COMPUTADORES',
        'ARA0045 - ENGENHARIA, SOCIEDADE E SUSTENTABILIDADE','ARA0015 - CÁLCULO DIFERENCIAL E INTEGRAL','ARA0020 - GEOMETRIA ANALÍTICA E ÁLGEBRA LINEAR',
        'ARA0038 - REPRESENTAÇÃO GRÁFICA PARA PROJETO','ARA0048 - FÍSICA TEÓRICA EXPERIMENTAL - MECÂNICA','ARA1386 - SISTEMAS OPERACIONAIS',
        'ARA0002 - PENSAMENTO COMPUTACIONAL','ARA0014 - ANÁLISE DE DADOS','ARA0018 - CÁLCULO DE MÚLTIPLAS VARIÁVEIS','ARA0044 - ELETRICIDADE E MAGNETISMO',
        'ARA0047 - FÍSICA TEÓRICA EXPER. - FLUIDOS, CALOR, OSCILAÇÕES','ARA1398 - MECÂNICA DOS SÓLIDOS','ARA0029 - ELETRICIDADE APLICADA',
        'ARA0030 - EQUAÇÕES DIFERENCIAIS','ARA0046 - FENÔMENOS DE TRANSPORTE','ARA0056 - QUÍMICA TECNOLÓGICA','ARA2042 - SISTEMAS DIGITAIS',
        'ARA0079 - COMUNICAÇÕES DE DADOS E REDES DE COMPUTADORES','ARA0083 - ELETRÔNICA ANALÓGICA','ARA0125 - CONTROLADORES LÓGICOS PROGRAMÁVEIS',
        'ARA1943 - MODELAGEM MATEMÁTICA','ARA0040 - BANCO DE DADOS','ARA0098 - ESTRUTURA DE DADOS','COMPILADORES',
        'ARA2545 - SISTEMAS DISTRIBUÍDOS E COMPUTAÇÃO PARALELA','ARA0095 - DESENVOLVIMENTO RÁPIDO DE APLICAÇÕES EM PYTHON',
        'ARA0141 - INSTRUMENTAÇÃO INDUSTRIAL','ARA0363 - PROGRAMAÇÃO DE SOFTWARE BÁSICO EM C','ARA2086 - ALGORITMOS EM GRAFOS',
        'ARA0301 - PROGRAMAÇÃO DE MICROCONTROLADORES','ARA0309 - LINGUAGENS FORMAIS E AUTÔMATOS','ARA1879 - AUTOMAÇÃO INDUSTRIAL',
        'ARA0085 - INTELIGÊNCIA ARTIFICIAL','ARA0115 - SISTEMAS EMBARCADOS','ARA1191 - SUP. DE ESTÁGIO E PRÉ-PROJETO EM ENG. DE COM.',
        'ARA1518 - ALGORITMOS DE PROCESSAMENTO DE IMAGEM','ARA0026 - TÓPICOS EM LIBRAS: SURDEZ E INCLUSÃO','ARA0154 - PROCESSOS INDUSTRIAIS E ROBÓTICA',
        'ARA0869 - INOVAÇÃO, EMPREENDE. E PROJETO FINAL - ENG DE COMP','ARA2074 - SEGURANÇA CIBERNÉTICA'
      ]
    };
    ...
    selectors.fields.dept?.addEventListener('change', () => {
      ...
      if (lista.length) {
        ...
        sel.disabled = false;
      } else { sel.innerHTML = '<option value="">Selecione o curso primeiro</option>'; sel.disabled = true; }
    });
  }
...
async function safeBuildOccupancyTable(filterDate) { try { await buildOccupancyTable(filterDate); } catch(e){ console.error('Tabela:', e); } }
...
async function buildOccupancyTable(filterDate) {
  ...
  const timeRanges=[]; let cur = new Date(slotStart);
  while (cur < slotEnd) { const next=new Date(cur); next.setMinutes(cur.getMinutes()+50); timeRanges.push(`${padHM(cur)}-${padHM(next)}`); cur=next; }
  ...
  if (!timeRanges.length || !labs.length) {
    table.innerHTML = `<tr><td class="p-4 text-center ${textClass}">Sem dados para exibir</td></tr>`; return;
  }
...
async function initOccupancyUpdates() {
  ...
  const dateInput = document.getElementById('occupancy-date'); if (!dateInput) return console.error('#occupancy-date não encontrado');
...
onReady(async () => {
  ...
  // logout
  document.getElementById('menu-logout-btn')?.addEventListener('click', () => { Auth.logout?.(); window.user = null; window.location.href = '../login.html'; });

  // btn notif
  const btnNotifs = document.getElementById('btn-ativar-notificacoes');
  btnNotifs?.addEventListener('click', () => { solicitarPermissaoNotificacao(); btnNotifs.disabled = true; btnNotifs.innerHTML = '<i class="fas fa-bell-slash"></i> Notificações Ativadas'; });
...
document.querySelectorAll('button.toggle-password').forEach(btn => {
  btn.addEventListener('click', () => {
    const input = btn.closest('.form-group')?.querySelector('input'); if (!input) return;
...
async function carregarHistoricoUsuarios() {
  const container = document.getElementById('lista-historico-usuarios'); if (!container) return console.error('#lista-historico-usuarios não encontrado');
  ...
  try {
    ...
    filtrados.sort((a,b) => ordenacao === 'updatedAt' ? (new Date(b.updatedAt)-new Date(a.updatedAt)) :
    ...
    filtrados.forEach(u => {
      html += `<div class="col-12 col-md-6 col-lg-4"><div class="card shadow-sm h-100"><div class="card-body">
        ...
        <p class="card-text mb-2"><small>Status: ${u.status==='approved'?'<span class="badge bg-success">Aprovado</span>':'<span class="badge bg-danger">Rejeitado</span>'}</small></p>
...
(function () {
  ...
  function mostrarToast(txt) {
    const body = document.getElementById('meuToastBody'); if (body) body.innerText = txt;
    const el = document.getElementById('toastNovoCadastro'); if (el) new bootstrap.Toast(el).show();
  }

  // --------- fetch admin com token + erros
  async function adminFetch(url, options = {}) {
    const token = localStorage.getItem('admin_token');
    if (!token) { alert('Sessão do Admin expirada. Faça login novamente.'); window.location.replace('/login.html'); return null; }
    try {
      ...
      // algumas rotas não retornam corpo
      const txt = await res.text();
      return txt ? JSON.parse(txt) : {};
    } catch (e) { console.error('adminFetch:', e); alert(e.message); return null; }
  }
  ...
  async function carregarUsuariosPendentes() {
    try {
      ...
      if (ultimoCountUsuarios === null && dados.length > 0) { mostrarToast(`${dados.length} usuário(s) pendente(s) no momento.`); if (podeNotificar) enviarNotificacao('🆕 Usuários Pendentes', `Existem ${dados.length} usuário(s) aguardando aprovação.`); }
      else if (ultimoCountUsuarios !== null && dados.length > ultimoCountUsuarios) { const diff = dados.length - ultimoCountUsuarios; mostrarToast(`${diff} nova(s) solicitação(ões) de usuário!`); if (podeNotificar) enviarNotificacao('🔔 Nova(s) Solicitação(ões) de Usuário', `${diff} novo(s) usuário(s) aguardando aprovação.`); }
  ...
  function renderizarUsuariosPendentes() {
    const container = document.getElementById('lista-pendentes-usuarios'); if (!container) return;
    ...
    if (!filtrados.length) { container.innerHTML = `<div class="text-center py-5 text-light"><i class="fas fa-user-clock fa-3x mb-3"></i><h4>Nenhuma solicitação de usuário pendente</h4><p>Não há novos usuários aguardando aprovação.</p></div>`; return; }

    let html = '<div class="row gx-3 gy-4">';
    exibidos.forEach(u => {
      const id = u._id || u.id; // segurança extra
    ...
    if (totalPag > 1) {
      ...
      for (let p=1;p<=totalPag;p++) html+=`<li class="page-item ${paginaAtualUsuarios===p?'active':''}"><a class="page-link" href="#" onclick="mudarPaginaUsuarios(${p})">${p}</a></li>`;
      html += `<li class="page-item ${paginaAtualUsuarios===totalPag?'disabled':''}"><a class="page-link" href="#" onclick="mudarPaginaUsuarios(${paginaAtualUsuarios+1})">&raquo;</a></li></ul></nav>`;
    }
    container.innerHTML = html;
  }

  function mudarPaginaUsuarios(p) { paginaAtualUsuarios = Math.max(1, p); renderizarUsuariosPendentes(); document.getElementById('lista-pendentes-usuarios')?.scrollIntoView({behavior:'smooth'}); }
  ...
  async function rejeitarUsuario(id) {
    ...
    const ok = await adminFetch(`${BASE_API}/api/admin/reject-user/${id}`, { method: 'PATCH' });
    if (ok !== null) carregarUsuariosPendentes();
  }
  ...
  async function carregarReservasPendentes() {
    try {
      ...
      if (ultimoCountReservas === null && dados.length > 0) { mostrarToast(`${dados.length} reserva(s) pendente(s) no momento.`); if (podeNotificar) enviarNotificacao('🆕 Reservas Pendentes', `Existem ${dados.length} reserva(s) aguardando aprovação.`); }
      else if (ultimoCountReservas !== null && dados.length > ultimoCountReservas) { const diff = dados.length - ultimoCountReservas; mostrarToast(`${diff} nova(s) solicitação(ões) de reserva!`); if (podeNotificar) enviarNotificacao('🔔 Nova(s) Solicitação(ões) de Reserva', `${diff} nova(s) reserva(s) aguardando aprovação.`); }
  ...
  function renderizarReservasPendentes() {
    const container = document.getElementById('lista-pendentes-reservas'); if (!container) return;
    ...
    if (!filtrados.length) { container.innerHTML = `<div class="text-center py-5 text-light"><i class="fas fa-calendar-times fa-3x mb-3"></i><h4>Nenhuma solicitação de reserva pendente</h4><p>Não há novas solicitações de reserva.</p></div>`; return; }
    ...
    if (totalPag > 1) {
      ...
      for (let p=1;p<=totalPag;p++) html+=`<li class="page-item ${paginaAtualReservas===p?'active':''}"><a class="page-link" href="#" onclick="mudarPaginaReservas(${p})">${p}</a></li>`;
      html += `<li class="page-item ${paginaAtualReservas===totalPag?'disabled':''}"><a class="page-link" href="#" onclick="mudarPaginaReservas(${paginaAtualReservas+1})">&raquo;</a></li></ul></nav>`;
    }
    container.innerHTML = html;
  }

  function mudarPaginaReservas(p) { paginaAtualReservas = Math.max(1,p); renderizarReservasPendentes(); document.getElementById('lista-pendentes-reservas')?.scrollIntoView({behavior:'smooth'}); }
  ...
  async function rejeitarReserva(id) {
    ...
    const ok = await adminFetch(`${BASE_API}/api/admin/reject-reservation/${id}`, { method: 'DELETE' });
  ...
  async function concluirReservation(id) {
    try { await adminFetch(`${BASE_API}/api/reservations/${id}`, { method: 'PUT', body: JSON.stringify({ status: 'concluido' }) }); }
  ...
  async function carregarReservasAtivas() {
    try {
      ...
      todas.forEach(r => { const fim = new Date(`${r.date}T${r.end}:00`); if (agora > fim) concluirReservation(r._id || r.id); });
      ...
      const filtradas = todas.filter(r => {
        const fim = new Date(`${r.date}T${r.end}:00`); if (agora > fim) return false;
        if (filtroData && r.date !== filtroData) return false;
  ...
  function renderizarReservasAtivas(reservas) {
    const container = document.getElementById('lista-ativas'); if (!container) return;
    container.innerHTML = '';
    const agora = new Date();
    reservas.forEach(r => {
      const inicio = new Date(`${r.date}T${r.start}:00`);
      const fim = new Date(`${r.date}T${r.end}:00`);
      let perc = 0; if (agora < inicio) perc = 0; else if (agora > fim) perc = 100; else perc = ((agora - inicio) / (fim - inicio)) * 100;
...
let _onPopupOk = null;
function showPopup(msg, onOk) { if (!popupModal || !popupText) return alert(msg); popupText.textContent = msg; _onPopupOk = onOk || null; popupModal.classList.remove('opacity-0','pointer-events-none'); }
popupOk?.addEventListener('click', () => { popupModal.classList.add('opacity-0','pointer-events-none'); if (_onPopupOk) _onPopupOk(); });
...
// ──────────────────────────────────────────────────
{
  ...
  btn2?.addEventListener('click', () => { root2.classList.toggle('dark'); localStorage.setItem('theme', root2.classList.contains('dark') ? 'dark' : 'light'); });
}
...
onReady(() => {
  ...
  form.addEventListener('submit', async e => {
    ...
    try { await Auth.register({ name, email, password: pwd, matricula }); showPopup('✅ Cadastro enviado! Aguardando aprovação em até 24h.', () => window.location.href = '/login.html'); }
    catch (err) { showPopup(err.message || 'Falha no cadastro'); }
  });

  // Olhinho senha
  document.querySelectorAll('button.toggle-password').forEach(btn => {
    btn.addEventListener('click', () => {
      const inp = btn.closest('.form-group')?.querySelector('input'); if (!inp) return;
      const icon = btn.querySelector('i');
